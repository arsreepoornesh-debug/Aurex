...
import { Sparkles, PlusCircle, Search, Check, X, User, DollarSign } from 'lucide-react';
...
export default function PremiumPackagesPage() {
  ...
  const [form, setForm] = useState({
    name: 'Premium 1:1 Clinical Rehabilitation (12 Sessions)',
    serviceType: 'PREMIUM',
    sessionCount: 12,
    price: 48000,
    validityDays: 60,
  ...
  async function loadPackages() {
    ...
    try {
      ...
      setPackages(all.filter((p: any) => p.serviceType === 'PREMIUM'));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="p-6 space-y-4 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-black text-slate-900 tracking-tight">Premium Packages (1:1 Clinical)</h1>
          <p className="text-xs text-slate-500">Master catalog for dedicated 1:1 specialist-led exercise physiology</p>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="py-10 text-center text-slate-400">No Premium packages defined.</td>
                </tr>
              ) : (
                packages.map((pkg, idx) => (
                  <tr key={pkg.id} className={`hover:bg-slate-50/80 transition-colors ${idx % 2 === 1 ? 'bg-slate-50/30' : 'bg-white'}`}>
                    ...
                    <td className="py-3 px-3 text-slate-600 font-medium">1:1 Dedicated Specialist</td>
